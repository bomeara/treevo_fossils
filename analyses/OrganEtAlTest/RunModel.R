...
niter.goal=5
	...
	cat(paste("\n",niter.white.g, "for white noise"))
		...
		trueFreeValues<-trueInitial[freevector]
			sim.time <- system.time(simTraits<-doSimulationWithPossibleExtinction(taxon.df=taxon.df, intrinsicFn=intrinsicFn, extrinsicFn=extrinsicFn, startingValues=trueStarting, intrinsicValues=trueIntrinsic, extrinsicValues=trueExtrinsic, timeStep=timeStep, verbose=TRUE))
print(sim.time)
		summary.time <- system.time(summaryStatsLong(phy, simTraits, niter.brown.g, niter.lambda.g, niter.delta.g, niter.OU.g, niter.white.g))
print(summary.time)
